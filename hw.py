...
def main():
    ...
    user_view.show_info("Welcome to the assistant bot!")
    while True:
        user_input = input("Enter a command: ")
        command, *args = parse_input(user_input)

        if command in ["close", "exit"]:
            user_view.show_info("Good bye!")
            with open(file_path, "wb") as file:
                pickle.dump(book, file)
            break

        elif command == "hello":
            user_view.show_info("How can I help you?")

        elif command == "add":
            user_view.show_info(add_contact(args, book))

        elif command == "change":
            user_view.show_info(change_contact(args, book))

        elif command == "phone":
            user_view.show_info(show_phone(args, book))

        elif command == "all":
            user_view.show_info(show_all(book))

        elif command == "add-birthday":
            user_view.show_info(add_birthday(args, book))

        elif command == "show-birthday":
            user_view.show_info(show_birthday(args, book))

        elif command == "birthdays":
            birthdays = book.get_upcoming_birthdays()
            if not len(birthdays):
                user_view.show_info("There are no upcoming birthdays.")
                continue
            for day in birthdays:
                user_view.show_info(f"{day}")

        else:
            user_view.show_info("Invalid command.")
